...
namespace ERP.DataAccessLayer
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<List<UserFeatureAccessModel>> GetMenuList(int UserId)
        {
            try
            {
                using (var dbConnection = new SqlConnection(_settings.ConnectionString[DbConnections.ERPDbContext.ToString()]))
                {
                    ...
                    var list = (await dbConnection.QueryAsync<UserFeatureAccessModel>("Usp_GetUserAccessMenuList", dynamicParameters, commandType: CommandType.StoredProcedure)).ToList();
                    return list;
